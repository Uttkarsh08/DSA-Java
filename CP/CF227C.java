...
public class CF227C {
    ...
    public static void Sol(long n, int from, int to, int using) {
        ...
        //if largest(4) is first to move to A , we cant move smaller elements(1 2 3) to A, 
        //because a number larger then them(4) is already on A, and then we have to again move largest elemet on A(4) to put smaller elements on A
        //But, to move smallest on A(1), we have to first move all elements larger then it(2 3 4) to A
        Sol(n - 1, from, to, using);  // move  2 3 4 from  C to A
        ans++;  // move 1 from C to B(we cant directly move from C to A beacuse we can move 1 step(to adjacent only)
    ...
    public static int power(int b, int e) {
        if (e == 0) return 1; // Base case: any number to the power of 0 is 1
